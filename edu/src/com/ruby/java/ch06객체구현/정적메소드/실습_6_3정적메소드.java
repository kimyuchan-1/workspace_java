...
class Student {
	...
	static void getNumberStudents(Student[] students) {
	...
	public static void printSubjectStats(Student[] students) {
		...
		for (int j = 0; j < max.length; j++) {
			System.out.println(students[0].subjects[j]+" 최대 점수: "+max[j]+"/"+maxStd[j]+", 최소 점수: "+min[j]+"/"+minStd[j]);
		}
	}
}
public class 실습_6_3정적메소드 {
	...
	public static void main(String[] args) {
		...
		// 학생 수를 정적 메소드 호출로 처리
		Student.getNumberStudents(students);
