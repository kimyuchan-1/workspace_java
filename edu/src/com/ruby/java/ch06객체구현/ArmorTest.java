...
public class ArmorTest {
	...
	public static void main(String[] args) {
		...
		String str; // String은 java가 가지고 있는 class, class의 로컬 변수는 참조변수 -> str: 참조변수
		str = "Hello"; // String literal은 method 영역의 constant pool에 저장
		System.out.println(str);
		
		int result = sum(10, 20);
		System.out.println(result);
		Armor am; // am은 로컬 변수로서 참조변수, 아직 객체는 없음
		am = new Armor(); // 생성자 호출, am이 객체를 가지게 됨
		System.out.println(am.getName() + " : " + am.getHeight());
		
		// 1. am이라는 로컬 변수 생성(stack 영역)
		// 2. heap 영역에 메모리 할당
		// 3. 생성자 호출 class의 필드 확인하여 heap 영역에 name과 height 저장
		// 4. 할당된 heap 영역의 메모리 주소를 am에 반환
		
		// 아래 코드는 무엇이 문제인가?
		new Armor();
		new Armor();
		new Armor();
		// 참조할 변수가 없음 -> java garbage collection이 수거함(삭제)
		
		int n = 10;
		Armor a; // 클래스로 변수 선언시 초기화가 필요하다.
		//a.height = 0; // not visible, Armor의 height는 private이기 때문
		a = new Armor();
		a.setHeight(n);
		/*
		 ...
		 */
		System.out.println(a.getName() + " : " + a.getHeight());//220페이지 표 참조
