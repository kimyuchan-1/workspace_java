...
abstract class Employee extends Object { // 모든 클래스는 최상위 클래스로 Object를 가짐, 자동으로 extends Object 생성
	String name;
	int salary;
	public Employee() {
		name= null;
	...
	@Override // Object 클래스의 toString()을 override
...
public class HRSTest {
	...
	*/
	...
	public static void calcTax(Employee e) {
		e.calcSalary();//동적 바인딩, 메소드 body를 결정하는 것을 바인딩이라 하고, e의 타입에 의해 호출되는 메소드가 결정되기 때문에 동적 바인딩
		
		double tax = e.salary * 0.1;
		System.out.println("세금 = " + tax);
		
		if (e instanceof Salesman) { // instanceof: e의 타입을 확인
			System.out.println("Salesman 인스턴스");
			//System.out.println(e.eno);//오류 > 353페이지 - 355페이지 그림 이해, Employee를 상속받는 모든 클래스가 eno 필드를 가지지 않기 때문
			...
			System.out.println(e.name);//오류가 없는 이유는? name 필드는 Employee가 가지는 필드이기 때문
		}  else {
	...
	public static void main(String[] args) {
		...
		Salesman s5 = s;
		if (s5.equals(s)) System.out.println(s5+"은 "+s+"와 같습니다.");
