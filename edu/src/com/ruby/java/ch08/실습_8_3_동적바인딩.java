...
class Order extends SeasonalDiscount {
	...
	private double calculateDiscountTotal() {
		 ...
		 * 할인을 적용한 비용 계산
		 */
		double DiscountTotal = calculateTotal() - (super.getDiscountedPrice(calculateTotal())) - (customer.getDiscountedPrice(calculateTotal()));
	...
	public void printOrderSummary() {
		...
		for (int i = 0; i < count; i++) {
			System.out.println("제품명 : "+items[i].getName()+", 단가 : "+items[i].getPrice()+", 개수 : "+quantities[i]+" ==> 가격 : "+items[i].getPrice()*quantities[i]+", 주문일 : "+orderDates[i]);
		}
		System.out.println("총액 : "+String.format("%.1f",calculateTotal()));
	}
